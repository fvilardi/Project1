...
function Cart(props) {
    ...
    const userShoppingCart = { id: 4, workShopId: '3', inventoryId: '9', quantity: cartItems.length, userId: 227 };
    const shoppingCartData = { itemsPrice, taxPrice, shippingPrice, totalPrice, id: 4, quantity: cartItems.length };
    ...
    const handleCheckout = () => {
        navigate(`/cart/checkout/${userShoppingCart.id}`, { state: { shoppingCartData } });
    };
    const mapCartData = (item) => {
        return (
            <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1rem' }}>
                <img
            ...
            {cartItems.length !== 0 && (
                <>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1rem' }}>
                        ...
                        <div style={{ flex: 1, textAlign: 'right' }}>{itemsPrice.toFixed(2)} AED</div>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1rem' }}>
                        <div style={{ flex: 2 }}> Tax </div>
                        <div style={{ flex: 1, textAlign: 'right' }}>{taxPrice.toFixed(2)} AED</div>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1rem' }}>
                        <div style={{ flex: 2 }}> Shipping </div>
                        <div style={{ flex: 1, textAlign: 'right' }}>{shippingPrice.toFixed(2)} AED</div>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1rem' }}>
                        <div style={{ flex: 2 }}>
                            <h4>Total </h4>
